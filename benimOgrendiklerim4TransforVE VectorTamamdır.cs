    ...
    */
     ...
     //  resim.transform.Rotate(yYonu, 120f * m_deltaTime, Space.World);
        ...
        */
        ...
        #region transform.RotateAround ( ) updare Metoduna yaz.

        /* RotateAround ( ) = Etrafýnda döndürmek.
           Bu dönüþümün( deðiþimin ) HEM KONUMUNU HEM DÖNÜÞÜNÜ deðiþtirir ! !
           Ekseni belirlerken  ÖRNEÐÝN  Rotate ( ) metodunda Vector3.Up Y ekseninden DÖNERKEN nasýl döndüðünü HATIRLA. Diðer eksenlerdende nasýl döndüðünü HATIRLAKÝ burda daha iyi iþlem yapabilesin.  



        
        Vector3 belirledigimNokta = new Vector3(2f, 2f, 2f);

          anlayisObjesi.transform.RotateAround(belirledigimNokta, Vector3.up, 32f*Time.deltaTime);
                  Anlayis Objesi  BelirlediðimNoktada dönerken PositionY 'si SABÝT KALIR. ÇÜNKÜ Y sÝ SABÝT KALSIN DEMÝÞ OLDUK KODU YAZARKEN ! !
                  Rotasyon olarak ise anlayisObjesinin SADECE RotationY'si DEÐÝÞÝR ! !
        

        anlayisObjesi.transform.RotateAround(belirledigimNokta, Vector3.right, 32f * Time.deltaTime);
                  Anlayis Objesi Belirlediðim noktada dönerken PositionX ' Si SABÝT KALIR. Çünkü X 'Ý SABÝT KALSIN DEMÝÞ OLDUK KODU YAZARKEN ! !
                  Rotasyon olarak ise anlayisObjesinin SADECE RotationX 'si DEÐÝÞÝR ! !

            

        anlayisObjesi.transform.RotateAround(belirledigimNokta, Vector3.forward, 32f * Time.deltaTime);
                  Anlayis objesi Belirlediðim noktada dönerken PositionZ 'si SABÝT KALIR. Çünkü Z 'si SABÝT KALSIN DEMÝÞ OLDUK KODU YAZARKEN ! !
                  Rotasyon olarak ise anlayisObjesinin SADECE RotationZ 'si DEÐÝÞÝR.
        */

        #endregion


        #region transform.SetAsFirstSibling ( )

        //  anlayisObjesi.transform.SetAsFirstSibling();
        //  Dusunce.transform.SetAsFirstSibling();

        // Hierarchy de nerde olursa olsun objeyi Hierarchy 'nin EN BAÞINA ATAR.
        ...
        // DunyaUzayýKonumObjesi. transform.SetAsLastSibling();
        // Hierarchy 'de nerde olursa olsun Hierarchy 'nin EN ALTINA atar.
        ...
        // Kodlarý çalýþtýrmadan önce sahnede Tablo objesinin Transformunu RESETLEDÝK.

        //    tablo.transform.SetParent(anlayisObjesi.transform,false);  // tablo objesinin Annesi anlayisObjesi ve Anlayis objesinin konumuyla AYNI OLDU ! !

        //  tablo.transform.SetParent(anlayisObjesi.transform, true); // tablo objesi KENDÝ KONUMUNU KORUDU yani Anlayis objesinin konumuyla AYNI OLMADI ! ! 


        #endregion


        #region transform.SetSiblingIndex ( ) 

        /* Birden fazla çocuklu bir aile varsa bu çocuklarýnýn SIRALAMASINI ayarlamamýzý saðlar.
           Anne cocuk iliþkisi yoksa YANÝ obje tekse HÝERARCHY 'deki SIRALAMASINI ayarlamamýzý saðlar.

        tablo.transform.SetSiblingIndex(1);
        ...
        */
        ...
        #region transform.TryGetComponent ( )

        //  Ýlgili objede þu component var mý bu component var mý diye kontrol eder.
        // Debug.Log (    transform.TryGetComponent(out DenemeninRigidbodysi) );
